...
(() => {
  ...
  /* --------------------------------------------------------------------------
     DOM helpers
     -------------------------------------------------------------------------- */
  const KEN_BURNS = ["kb-zoom-in", "kb-pan-right", "kb-zoom-out"];
  ...
  const restartKenBurns = (frame) => {
    if (!frame || prefersReducedMotion()) return;
    frame.classList.remove("is-living");
    void frame.offsetWidth;
    frame.classList.add("is-living");
  };
  ...
  const updateChrome = (index) => {
    state.slides.forEach((frame, i) => {
      ...
      frame.classList.toggle("is-living", false);
      frame.setAttribute("aria-hidden", active ? "false" : "true");
      if (active) restartKenBurns(frame);
    });
